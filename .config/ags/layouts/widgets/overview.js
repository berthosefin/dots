...
function substitute(str) {
  const subs = [{ from: "Caprine", to: "facebook-messenger" }];
...
const client = ({ address, size: [w, h], class: c, title }) =>
  Widget({
    ...
    tooltip: title,
    onSecondaryClick: () =>
      execAsync("hyprctl dispatch closewindow address:" + address).catch(print),
    setup: (button) => {
...
const workspace = (index) => {
  ...
  widget.update = (clients) => {
    ...
    clients = clients.map((client) => {
      const [x, y] = client.at;
      if (x > 1366) client.at = [x - 1366, y];
      return client;
    });

    fixed.get_children().forEach((ch) => ch.destroy());
    clients.forEach(
      (c) => c.mapped && fixed.put(client(c), c.at[0] * SCALE, c.at[1] * SCALE)
    );
    fixed.show_all();
  };
  return widget;
...
Widget.widgets["overview"] = ({ workspaces = 9, windowName = "overview" }) =>
  Widget({
    type: "box",
    className: "overview",
