...
const pinned = ["wezterm", "thunar", "firefox", "rhythmbox", "code-oss"];
      ...
      connections: [
        [
          ...
          (button) => {
            let running = false;
            for (const [, client] of Hyprland.clients) {
              if (client.class.toLowerCase().includes(term)) running = client;
            }

            button.toggleClassName("nonrunning", !running);
            button.toggleClassName(
              "focused",
              Hyprland.active.client.address === running.address?.substring(2)
            );
      ...
      }),
      {
        ...
        connections: [
          [
            ...
            (box) => {
              box.visible =
                box.get_parent().get_children()[launcher ? 3 : 2].get_children()
                  .length > 0;
            },
          ],
        ],
        ...
        item: ({ iconName }, { address, title }) => ({
          ...
          className:
            Hyprland.active.client.address === address.substring(2)
              ...
              : "nonfocused",
          onClick: () =>
            execAsync(`hyprctl dispatch focuswindow address:${address}`).catch(
              print
            ),
        }),
      },
