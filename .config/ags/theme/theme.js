...
class ThemeService extends Service {
  ...
  openSettings() {
    if (!this._dialog) this._dialog = imports.theme.dialog.dialog();
  ...
  setupOther() {
    ...
    const colorsch = this.getSetting("theme");

    switch (colorsch) {
      case "nord":
        execAsync(`sh /home/${USER}/.config/hypr/bin/nord.sh`).catch(print);
        break;
      case "dracula":
        execAsync(`sh /home/${USER}/.config/hypr/bin/dracula.sh`).catch(print);
        break;
      case "gruvbox":
        execAsync(`sh /home/${USER}/.config/hypr/bin/gruvbox.sh`).catch(print);
        break;
      default:
        execAsync(`sh /home/${USER}/.config/hypr/bin/nord.sh`).catch(print);
    }
    ...
    execAsync(`${gsettings} "prefer-${darkmode ? "dark" : "light"}"`).catch(
      print
    );
...
Widget.widgets["theme/indicator"] = (props) =>
  Widget({
    ...props,
    type: "stack",
    transition: "crossfade",
    items: Object.values(themes).map(({ name, icon }) => [
      name,
      {
        type: "font-icon",
        icon,
    ...
    connections: [
      [Theme, (stack) => stack.showChild(Theme.getSetting("theme"))],
    ],
  });

Widget.widgets["theme/toggle"] = (props) =>
  Widget({
    ...props,
    type: "button",
    className: "active",
...
Widget.widgets["theme/selector"] = (props) =>
  Widget({
    ...props,
    type: "box",
        ...
        children: [
            ...
            .join(" "),
          {
            ...
            connections: [
              [
                Theme,
                (icon) => (icon.visible = Theme.getSetting("theme") === name),
              ],
            ],
          },
        ],
